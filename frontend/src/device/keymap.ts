...
// CCOS action codes from Serial API documentation
// https://docs.charachorder.com/SerialAPI.html
export const KEYMAP_CODES: Map<number, KeymapAction> = new Map([
...
/**
 * Get a human-readable name for an action ID.
 * Falls back to ASCII character for printable range (32-126)
 * or "Unknown(id)" for unknown codes.
 */
export function getActionName(actionId: number): string {
  const action = KEYMAP_CODES.get(actionId);
  if (action) {
